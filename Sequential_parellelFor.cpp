...
void HistogramThresolding(CImg<float> imgin, CImg<float>* imgout,  size_t nWorkers, float threshold)
{
    // RGBtoGrayScale
    auto start1 = std::chrono::high_resolution_clock::now();
    *imgout = RGBtoGrayScale(imgin, nWorkers);
...
int main(int argc, char * argv[]) 
{
  ...
  while(true)
  {
     if(nImages)
     {
        ...
        HistogramThresolding(imgin,&imgout,nWorkers,threshold);
        nImages--;
  ...
  std::cout << "nWorkers : " << nWorkers << " TotalTime : " << std::chrono::duration_cast<std::chrono::milliseconds>(end-start).count()<< " (ms)" <<std::endl;
