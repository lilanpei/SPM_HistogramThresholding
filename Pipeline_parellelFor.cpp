...
struct firstStage: ff_node_t<Task_t> {
    ...
    size_t nImages;
    float threshold;
    firstStage(CImg<float> imgin, size_t nImages, float threshold)
...
int main(int argc, char * argv[]) 
{
  ...
  ffTime(STOP_TIME);
  std::cout <<"read the image: " << ffTime(GET_TIME) << " (ms)" <<std::endl;

  firstStage   Emitter(imgin,nImages,threshold);
  secondStage  RGBtoGrayScale;
  thirdStage   Histogram;
  fourthStage  Thresholding;

  // builds the pipeline
  ff_Pipe<> pipe(Emitter,RGBtoGrayScale,Histogram,Thresholding);
  pipe.wrap_around();
  // starts the pipeline
